...
if ( ! class_exists( 'Organic_Farm_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Organic_Farm_WPTT_WebFont_Loader {
		...
		public function get_remote_url_contents() {
			...
			$organic_farm_contents = wp_remote_retrieve_body( $response );

			return $organic_farm_contents;
		}
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $organic_farm_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $organic_farm_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$organic_farm_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$organic_farm_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$organic_farm_font_family = trim( str_replace( array( "'", ';' ), '', $organic_farm_font_family ) );
					$organic_farm_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $organic_farm_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $organic_farm_font_family ] ) ) {
					$result[ $organic_farm_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$organic_farm_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$organic_farm_font_family_url = $this->get_absolute_path( $organic_farm_font_family_url );

					$result[ $organic_farm_font_family ][] = $organic_farm_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $organic_farm_font_family ] = array_flip( array_flip( $result[ $organic_farm_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Organic_Farm_WPTT_WebFont_Loader( $url );
